...
function isStartGroup(t): t is ComplexRegexToken {
  return (t && !!t.startGroup) || false;
}
function isEndGroup(t): t is ComplexRegexToken {
  return (t && !!t.endGroup) || false;
}
...
class RegexParser {
  ...
  static parseChars(template, i, mode) {
    ...
    while (template[i] != "]") {
      ...
      } else token.tokens.push(template[i]);
      i++;
    }
    ...
    while (i < template.length) {
      ...
      if (template[i] == "^" && tokens.length == 0) tokens.push(FIXED_START);
        ...
        if (typeof last === "string")
          tokens.push({
            anyOf: true,
            tokens: [NOTHING, { tokens: [last], oneOrMore: true }]
          });
        else
          tokens.push({
            anyOf: true,
        ...
        tok &&
          tokens.push({
            tokens: [{ startGroup: group }, tok, { endGroup: group }]
          }) &&
          (i += tok.template.length);
        group = nextGroup;
        ...
        i++,
          ([tok, i] = this.parseChars(template, i, "anyOf")) &&
            tok &&
            tokens.push(tok);
      else tokens.push(template[i]);

      i++;
    }
    if (rootToken.anyOf)
      rootToken.tokens.push(tokens.length ? { tokens: tokens } : NOTHING);
    else rootToken.tokens = tokens;
...
export class RegexBuilder {
  static convert(
    ...
    if (typeof token == "string" || token.anyCharExcept) {
      ...
      if (token.tokens.indexOf(NOTHING) > -1) nextFromState = [firstFromState];
      else nextFromState = [];
      for (let tok of token.tokens.filter(
        t =>
          t != NOTHING &&
          t != FIXED_START &&
          t != FIXED_END &&
        ...
        if (result.nextFromState.length > 1)
          nextFromState = nextFromState.concat(
      ...
      for (let tok of token.tokens.filter(
        ...
        nextFromState = [].concat
          .apply([], results.map(r => r.nextFromState))
          .removeDuplicates();
        finalState = results
          .map(r => r.finalState)
  ...
  static normalize(transitions: Transition[], finalStates: number[]) {
        ...
        if (
          ...
          if (
            ...
            addedTransitions.push({
              ...
              toState: anyCharT.toState,
              token: charT.token,
              startGroup: anyCharT.startGroup,
              endGroup: anyCharT.endGroup
            });
        }
      }
    }
    ...
    while (queue.length) {
      ...
      for (let tr of trgroup.filter(t => !!t.token)) {
        ...
        let closure = transitions.filter(
          t => reachableStates.indexOf(t.fromState) > -1
        );
        let closureId = ensureId(closure);
        let sameTokenTransactions = trgroup.filter(
          t => JSON.stringify(tr.token) === JSON.stringify(t.token)
        );
        ...
        states[id].transitions.push({
          condition: tr.token,
          next: closureId,
          fixedStart: tr.fixedStart,
          fixedEnd: tr.fixedEnd,
          startGroup: startGr,
          endGroup: endGr
        });
    ...
    for (let state of states) {
      ...
      if (charTransitions.length > 1) {
        ...
        let condition = {
          fromChar: charTransitions[0].condition,
          toChar: charTransitions[0].condition
        };
        for (let i = 1; i <= charTransitions.length; i++) {
          if (
            ...
            JSON.stringify(charTransitions[i].endGroup) ==
              JSON.stringify(charTransitions[i - 1].endGroup)
          ) {
            ...
            if (i < charTransitions.length)
              condition = {
                fromChar: charTransitions[i].condition,
                toChar: charTransitions[i].condition
              };
          }
        }
        state.transitions = classTransitions.concat(
          state.transitions.filter(t => typeof t.condition != "string")
        );
      }
    }
    return states;
