let match = "something here".match(/thing/);
console.log(match[0]);
match = "Matching string".match(/t[es]+t/);
if (match) console.log("Incorrect!");
...
let startTagRegex = /^<(!?[-A-Za-z0-9_]+)((?:\s+[\w\-\:]+(?:\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|[^>\s]+))?)*)\s*(\/?)>/;

let match2 = html.match(startTagRegex);
if (match2) console.log(match2);

console.log('<img src="test.png" />'.match(startTagRegex));

let endTagRegex = /^<\/([-A-Za-z0-9_]+)[^>]*>/;
